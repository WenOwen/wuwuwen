...
def integrate_sector_data():
    ...
    logger.info(f"✅ 找到对应数据的股票: {len(available_stocks)}只")
    if missing_stocks:
        logger.warning(f"⚠️  缺失数据的股票: {len(missing_stocks)}只")
    ...
    # 过滤出有数据的股票
    df_training = df_mapping[df_mapping['股票代码'].isin(available_stocks)].copy()
    ...
    df_training['primary_concept'] = df_training['概念板块'].apply(extract_primary_concept)
    
    # 重命名列以符合代码期望
    df_training = df_training.rename(columns={
        '股票代码': 'stock_code',
        '股票名称': 'stock_name', 
        '所属行业': 'industry',
        '概念板块': 'all_concepts',
    ...
    with open(features_file, 'w', encoding='utf-8') as f:
        json.dump(additional_features, f, ensure_ascii=False, indent=2)
    ...
    with open(summary_file, 'w', encoding='utf-8') as f:
        ...
        f.write(f"- 有数据股票: {len(available_stocks)}只\n")
...
if __name__ == "__main__":
    ...
    print("\n整合结果:")
    print(f"✅ 可用股票: {result['available_stocks']}只")
