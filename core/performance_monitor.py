...
class AutoOptimizer:
    ...
    def optimize_model_weights(self, prediction_days: int) -> Dict:
        ...
        if base_accuracy > 0.6:
            # 性能好时，保持当前权重分布
            new_weights = {'LSTM': 0.4, 'LightGBM': 0.3, 'Transformer': 0.2, 'CNN-LSTM': 0.1}
        elif base_accuracy > 0.55:
            # 性能中等时，增加LightGBM权重
            new_weights = {'LSTM': 0.35, 'LightGBM': 0.35, 'Transformer': 0.2, 'CNN-LSTM': 0.1}
        else:
            # 性能较差时，更依赖传统方法
            new_weights = {'LSTM': 0.3, 'LightGBM': 0.4, 'Transformer': 0.2, 'CNN-LSTM': 0.1}
